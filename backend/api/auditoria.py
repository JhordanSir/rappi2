...
async def list_auditoria(
    ...
    return await auditoria_service.buscar(mongo_db, actor, metodo, skip, limit)
...
async def resumen_auditoria(
    ...
    """Resumen agregado: requests por status, por metodo, top rutas, top actores."""
    ...
    pipeline_usuarios = [
        {"$match": {**base_match, "actor": {"$ne": None}}},
    ...
    top_usuarios = [{"actor": d["_id"], "requests": d["n"]} async for d in coll.aggregate(pipeline_usuarios)]
