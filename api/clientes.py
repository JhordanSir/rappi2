...
from schemas.clientes import ClienteCreate, ClienteResponse, ClienteDireccionCreate, ClienteDireccionResponse
...
async def list_clientes(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    result = await db.execute(select(Cliente).options(selectinload(Cliente.direcciones)).offset(skip).limit(limit))
